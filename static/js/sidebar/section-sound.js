...
// === onclick for buttons ===
async function sidebarOnclickVolumeButtons(element, key, section, value, className) {
...
function sidebarOninputVolumeSlider(element, key, type='default') {
    ...
    if(type == 'volumeLevel') {
        ...
        const volumeSvgs = element.parentNode.parentNode.querySelectorAll('.sidebar-title_and_svg .sidebar-volumeSvgs .svg');
...
function sidebarOnchangeVolumeSlider(element, key) {
    const range_number = element.parentNode.querySelector('.range_number');
    const range_slider = element.parentNode.querySelector('.range_slider');
...
function sidebarApplyVolumeButtons() {
    let soundSection = document.querySelector('#sidebar-sound .content');
    ...
    for(const key in settings_state.sound) {
        ...
        if(k.type.includes('buttons')) {
            let soundButtons = domElements[key].querySelectorAll('.sidebar-global_settings_button > *');
        ...
        if(k.type.includes('volume-preview-and-slider')) {
            ...
            // number and slider
            let range_number = domElements[key].parentNode.querySelector('.slider_option .sidebar-range_container .range_number');
            let range_slider = domElements[key].parentNode.querySelector('.slider_option .sidebar-range_container .range_slider');
            ...
            const volumeSvgs = range_slider.parentNode.parentNode.querySelectorAll('.sidebar-title_and_svg .sidebar-volumeSvgs .svg');
...
function sidebarApplyVolumeSettings() {
    sidebarApplyVolumeButtons();
...
function sidebarApplyVolumeHome() {
